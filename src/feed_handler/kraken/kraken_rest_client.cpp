...
std::string JoinErrors(simdjson::ondemand::document& doc) {
    std::string joined;
...
std::string DescribeHttpFailure(std::string_view what, const ix::HttpResponsePtr& response) {
...
RestClient::RestClient(std::string base_url, std::filesystem::path nonce_state_file)
...
std::expected<WebsocketsToken, std::string> RestClient::FetchWebsocketsToken(
    ...
    const std::string post_data = EncodePostData(params);
    const auto signature =
        SignPrivateRequest(kWebSocketsTokenPath, nonce, post_data, creds.api_secret_b64);
    if (!signature) {
    ...
    if (!response || response->statusCode != 200) {
        return std::unexpected(response ? DescribeHttpFailure("GetWebSocketsToken", response)
    ...
    const std::string errors = JoinErrors(doc);
    if (!errors.empty()) {
...
std::expected<std::size_t, std::string> RestClient::LoadAssetPairs(std::string_view pair) {
    ...
    if (!response || response->statusCode != 200) {
        return std::unexpected(response ? DescribeHttpFailure("AssetPairs", response)
                                        : std::string("AssetPairs: no response"));
    }
    return ParseAssetPairs(response->body);
}

std::expected<std::size_t, std::string> RestClient::ParseAssetPairs(std::string_view body) {
    simdjson::ondemand::parser parser;
    ...
    for (auto field : result) {
        ...
        if (entry["tick_size"].get_string().get(text) == simdjson::SUCCESS) {
            ...
            parsed.tick_size_value = ParseDouble(parsed.tick_size);
        }

        const std::string rest_name = parsed.rest_name;
        pairs_.emplace(rest_name, std::move(parsed));

        const AssetPair& stored = pairs_.at(rest_name);
        IndexPair(rest_name, rest_name);
        IndexPair(rest_name, stored.altname);
        IndexPair(rest_name, stored.ws_name);
    }

    return pairs_.size();
}

void RestClient::IndexPair(const std::string& rest_name, std::string alias) {
    if (alias.empty()) {
        return;
    }
    aliases_.insert_or_assign(alias, rest_name);
    aliases_.insert_or_assign(NormalizeSymbol(alias), rest_name);
}

const AssetPair* RestClient::FindAssetPair(std::string_view name) const {
    ...
    const auto normalized = aliases_.find(NormalizeSymbol(name));
