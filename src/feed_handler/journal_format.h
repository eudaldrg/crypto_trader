...
//    offset size field
//     0      1   record_type      uint8, see record_type below
...
constexpr bool is_known_record_type(std::uint8_t raw) {
    return raw == static_cast<std::uint8_t>(record_type::wire_message) ||
           raw == static_cast<std::uint8_t>(record_type::connection_incarnation);
}

/// Writes `value` little-endian into the first sizeof(T) bytes of `out`.
/// Shift-based so it is correct on any host byte order.
template <typename T>
constexpr void store_le(std::span<std::byte> out, T value) {
...
template <typename T>
constexpr T load_le(std::span<const std::byte> in) {
    std::uint64_t bits = 0;
    for (std::size_t index = 0; index < sizeof(T); ++index) {
...
inline std::uint32_t crc32_of(std::span<const std::byte> data) {
    return crc32_update(kCrc32Seed, data);
}
