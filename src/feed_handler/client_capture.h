...
// the session, the destructor calls it, and the session is declared before the
// client that journals into it so it is destroyed after it. An exchange derives,
// says how to build its client, and adds its own Summary().
#pragma once
...
class ClientCapture : public CaptureConnection {
  ...
  protected:
    ...
    /// exchange tag, the files are named after the connection id.
